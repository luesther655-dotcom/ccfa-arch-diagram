...
> **设计全程严禁读取 `examples/` 里的任何文件**。`examples/` 只是维护者
> 回归校验用的成品样例（`scripts/validate.py` 用它们跑 0/0 回归），
> **不是设计模板**——照着样例会画成千篇一律的"同一张图"，做不出属于
> 用户模型自己的版面。选原型、配色、走线一律只从本文件与 `references/`
> 的**文字规范**出发，按用户模型**原创排布**。除非用户明确要求
> "参考某张示例 / 我上次的图 / 从这张图学配色"，否则一次也不要打开
> `examples/`。
...
`references/ccfa-style-guide.md` 第 1 节的**文字描述**；**设计时不要打开
`examples/` 里的样例文件**，它们只是维护者回归校验用，照着画会千篇一律）：

| 模型类型 | 原型 | 版面要点 |
|---|---|---|
| 单一网络骨干（CNN/Transformer/超分/ViT） | A 流水线+放大 | 水平长条主干 + vbar 竖条链 + op 小圆 + 底部斜体组标题 |
| 训练流程 / 持续学习 / 多阶段 | B 多面板训练图 | (a)训练 (b)迁移 (c)测试 面板 + 跨面板粗箭头 |
| 多 Agent / RAG / LLM 系统 | C 系统分区图 | 2×2 淡彩面板 + 粗色跨面板箭头 + 左侧栏 + 右侧图例列 |
| CoT / 推理链 / 规划 | D 推理阶段容器 | 阶段大面板 + 编号步骤模块 + note 便签产物 |
| 多模态大模型 / 统一框架 | E 横带框架图 | 能力面板并排 + 深色标题栏分组 + 底部训练带 |
...
0. **不读样例**：`examples/` 是维护者回归校验集，**设计全程严禁打开**；
   原型、配色、走线一律从本文件与 `references/` 的文字规范**原创**，
   禁止抄样例版面。除非用户明确要求参考某张示例图。
